...
Follow Test Driven Development methodology - Adhere to a strict `RED → GREEN → REFACTOR` cycle: write a failing test first, make it pass with the simplest implementation, then clean up while keeping the suite green.
...
## Testing Guidelines
Make sure to follow Test Driven Development
